...
// function SoLuGzJd() {	//此程式功能為顯示整月陽曆陰曆干支曆儒略日對照。此程式適用範圍為-1000至3000年
   var dStr1 = new Array('日', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十');
    ...
    function calElement(sYear, sMonth, sDay, week, lYear, lMonth, lDay, isLeap, cYear, cMonth, cDay, cZod,jdstr) {
        this.isToday = false;
        this.sYear = sYear;
        this.sMonth = sMonth;
        this.sDay = sDay;
        ...
        //將1582年10月的天數獨立處理
         ...
         for (kk = 0; kk <= dom - 1; kk++) {
             ...
             //'''''下面的指令中減0.5是為了改為從0時算起而不從正午算起   
                ...
                // document.write(out);
                if (n == 0) firstLM = mis;
                if (kk == 0) Ldpos[n++] = kk - di + 1;
                if (di == 1) Ldpos[n++] = kk - di + 1;
                //if(m==1 && (i+1)==term2) {
                //   Cy=GanZhi(y-1912+48);
             ...
             jdstr = jdscd +jdsgd+ jdsth + jdsyk + jdstrg;
                ...
                this[kk] = new calElement(yea, mx, pod,dStr1[wk], yi, mis, di, IL, CYY, CMM, CDD, Czod,jdstr);
